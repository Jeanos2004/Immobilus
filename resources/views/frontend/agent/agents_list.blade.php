...
<section class="page-title centred">
    <div class="title-outer">
        <div class="content-box">
            <h1>{{ __('messages.agents_list') }}</h1>
            <ul class="bread-crumb clearfix">
                <li><a href="{{ route('homepage') }}">{{ __('messages.home') }}</a></li>
                <li>{{ __('messages.agents_list') }}</li>
            </ul>
        </div>
    </div>
</section>
                ...
                <div class="agents-sidebar">
                    <div class="search-widget sidebar-widget">
                        ...
                        <div class="search-inner">
                            <form action="{{ route('agents.list') }}" method="get">
                    ...
                    <div class="category-widget sidebar-widget">
                        ...
                        <ul class="category-list clearfix">
                            <li><a href="{{ route('agents.list') }}">{{ __('messages.all_agents') }} <span>({{ App\Models\User::where('role', 'agent')->count() }})</span></a></li>
                            <li><a href="{{ route('agents.list', ['status' => 'active']) }}">{{ __('messages.active_agents') }} <span>({{ App\Models\User::where('role', 'agent')->where('status', 'active')->count() }})</span></a></li>
                            <li><a href="{{ route('agents.list', ['status' => 'inactive']) }}">{{ __('messages.inactive_agents') }} <span>({{ App\Models\User::where('role', 'agent')->where('status', 'inactive')->count() }})</span></a></li>
                        </ul>
                    </div>
                </div>
            </div>
